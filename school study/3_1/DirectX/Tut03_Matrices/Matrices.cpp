...
VOID Render()
{
	...
	if (SUCCEEDED(g_pd3dDevice->BeginScene()))
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			if (number1)
			{
				ioffset = timeGetTime() % 1000;

				fAngle = ioffset * (2.0f * D3DX_PI) / 1000.0f;
			...
			{
				checkx = nextPos.x - beforPos.x;
				checky = nextPos.y - beforPos.y;
				if (checkx >checky)
				{
					ioffset = checkx % 1000;
					fAngle = ioffset * (2.0f * D3DX_PI) / 1000.0f;
...
LRESULT WINAPI MsgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		break;
	case WM_LBUTTONDOWN:
		//GetCursorPos(beforPos);

		beforPos.x = LOWORD(lParam);
		beforPos.y = HIWORD(lParam);
		MouseDown = true;
	...
	case WM_LBUTTONUP:
		...
		break;
	case WM_MOUSEMOVE:
		//GetCursorPos(nextPos);
		if (MouseDown)
		{
			nextPos.x = LOWORD(lParam);
			nextPos.y = HIWORD(lParam);
		}
		break;
	case WM_DESTROY:
		Cleanup();
...
INT WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, LPWSTR, INT)
{
	...
	HWND hWnd = CreateWindow(L"D3D Tutorial", L"D3D Tutorial 03: Matrices",
		WS_OVERLAPPEDWINDOW, 100, 100, 1800, 1000,
